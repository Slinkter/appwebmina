...
function CreatePedido() {
    ...
    const handleDelete = (item) => {
        dispatch(removeItemFromOrder(item.docId));
    };
    ...
    function handleAddItem() {
        ...
        const existingItem = listItem.find(
            (item) => item.docId === currentSelectProduct.docId
        );

        const totalCantidad = existingItem
            ? existingItem.cantidad + cantidad
            : cantidad;

        if (totalCantidad <= 0 || Number.isNaN(cantidad)) {
            alert("La cantidad debe ser un número mayor a cero.");
            setCount("");
            return;
        }

        if (totalCantidad > currentSelectProduct.cantidad) {
    ...
    const handleSubmit = async () => {
        if (!selectedEmployer || listItem.length === 0) {
            ...
            try {
                ...
                await saveOrderAndDecreaseStock(orderValid);
    ...
    return (
        <DashboardWrapper>
            ...
            >
                <Box sx={{ my: 2 }}>
                    <Typography variant="h3">Crear Orden</Typography>
                </Box>
                <Container maxWidth="sm">
                    <form>
                        ...
                        >
                            {employers.length > 0
                                ? employers.map((obj) => (
                                      <option key={obj.docId} value={obj.dni}>
                                          {obj.dni
                                              ? " DNI : " +
                                                obj?.dni +
                                                " --- Nombre  : " +
